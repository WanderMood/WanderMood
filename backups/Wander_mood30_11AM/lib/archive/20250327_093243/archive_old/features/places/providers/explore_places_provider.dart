...
final explorePlacesProvider = StateNotifierProvider<ExplorePlacesNotifier, AsyncValue<List<Place>>>((ref) {
  final placesService = ref.watch(placesServiceProvider);
  return ExplorePlacesNotifier(placesService);
});

class ExplorePlacesNotifier extends StateNotifier<AsyncValue<List<Place>>> {
  final PlacesService _placesService;
  List<Place> _places = [];
  bool _hasMore = true;
  int _currentPage = 1;

  ExplorePlacesNotifier(this._placesService) : super(const AsyncValue.loading());
  ...
  Future<List<Place>> getNearbyPlaces({
    ...
    if (!_hasMore) return _places;

    try {
      state = const AsyncValue.loading();
      final newPlaces = await _placesService.getNearbyPlaces(
        latitude: latitude,
        longitude: longitude,
        category: category,
        page: page,
      ...
      _hasMore = newPlaces.length >= 20; // Assuming 20 is the page size
      _currentPage = page;
