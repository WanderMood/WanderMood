...
class NavigationService {
  static Future<void> navigateToLocationPreferences(BuildContext context) async {
    // Create a top-level function that doesn't depend on the build context
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('hasCompletedOnboarding', true);
    
    // Navigate using a new method to avoid build context issues
...
class _WelcomeScreenState extends ConsumerState<WelcomeScreen> with TickerProviderStateMixin {
  ...
  void _initButtonNavigation(BuildContext context) {
    // Using a VoidCallback for the button that doesn't directly access providers
    NavigationService.navigateToLocationPreferences(context).then((_) {
      // This only runs if navigation fails
      if (mounted) {
        setState(() {
          _isReadyForNextStep = true;
        });
                              ...
                              child: Container(
                                ...
                                child: ElevatedButton(
                                  onPressed: _isReadyForNextStep 
                                    ? () {
                                        setState(() => _isReadyForNextStep = false);
                                        debugPrint('Button pressed, initiating navigation...');
                                        // Use the callback approach
                                        _initButtonNavigation(context);
                                      }
                                    : null,
