...
class OpenAIService {
  ...
  Future<String?> generatePlaceDescription({
    ...
    try {
      final prompt = '''
        Describe $placeName (types: ${placeTypes.join(', ')}) 
        for someone feeling $userMood.
        Keep it brief, engaging, and mood-appropriate.
        Focus on why it's a good match for their current mood.
      ''';
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'messages': [
            {
              'role': 'system',
              'content': 'You are a travel writer that creates engaging, personalized place descriptions.',
            },
            {
              'role': 'user',
              'content': prompt,
