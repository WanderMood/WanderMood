...
class _ExploreScreenState extends ConsumerState<ExploreScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return locationState.when(
      data: (location) {
        ...
        final trendingDestinations = ref.watch(trendingDestinationsProvider(city: location, category: _selectedCategory));
        final explorePlaces = ref.watch(explorePlacesProvider(city: location, category: _selectedCategory));
  ...
  Widget _buildPlaceCard(PlacesSearchResult place) {
    ...
    // Calculate a more realistic distance for Rotterdam locations (1-15 km)
    final int distance = (place.placeId.hashCode.abs() % 14) + 1;
    
    // Get activity types based on place types
    final activities = _getActivitiesFromPlaceTypes(place.types ?? []);
    
    // Track favorite state with a ValueNotifier
    final isFavorite = ValueNotifier<bool>(false);
    
    // Get or generate price level if not available (1-4)
    final int priceLevel = place.priceLevel != null ? place.priceLevel as int : ((place.placeId.hashCode.abs() % 4) + 1);
    final priceRange = List.filled(priceLevel, '\$').join();
    
    // Generate opening hours info if not available
    final isOpen = place.openingHours?.openNow ?? (place.placeId.hashCode.abs() % 2 == 0);
    final closingHour = 6 + (place.placeId.hashCode.abs() % 12); // Between 6 PM and 6 AM
    final openingStatus = isOpen 
        ? "Open Now • Closes at ${closingHour > 12 ? closingHour - 12 : closingHour}${closingHour >= 12 ? 'PM' : 'AM'}" 
        : "Closed • Opens at 8AM";
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: Colors.white.withOpacity(0.85), // Make cards slightly see-through
                ...
                Positioned(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      // Favorite button with ValueListenableBuilder to update UI when toggled
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        ...
                        const SizedBox(width: 4),
                        Text(
                          '• ${_getPriceRangeDescription(priceLevel)}',
                          style: GoogleFonts.poppins(
                            fontSize: 13,
                  ...
                  // Description
                  Text(
                    place.vicinity ?? 'Experience the beauty of ${currentCity}',
                    style: GoogleFonts.poppins(
                      fontSize: 14,
                      color: Colors.grey.shade600,
                  ...
                  // Location and distance - simplified to just city and country flag
                  Row(
                    children: [
                      const Icon(
                        Icons.location_on,
                  ...
                  if (activities.isNotEmpty)
                    SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: Row(
                        children: activities.map((activity) => _buildActivityButton(
                          activity['emoji']!,
                          activity['name']!,
                        )).toList(),
                      ),
  ...
  Widget _buildActivityButton(String emoji, String name) {
    // Check if this is a hiking or sightseeing button to make it green
    final bool isHighlighted = name == "Hiking" || name == "Sightseeing";
    ...
    return Container(
      ...
      child: OutlinedButton.icon(
        ...
        style: OutlinedButton.styleFrom(
          side: BorderSide(
            color: isHighlighted ? Colors.green.shade500 : Colors.grey.shade200,
          ),
  ...
  void _sharePlace(PlacesSearchResult place) async {
    try {
      final locationState = ref.watch(locationProvider);
      final currentCity = locationState.value ?? 'Rotterdam';
      
      final message = 'Check out ${place.name} in $currentCity! Found via WanderMood app 🧳✨';
      await Share.share(message);
    } catch (e) {
      debugPrint('Error sharing: $e');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Could not share this place')),
      );
    }
  }
  
  void _toggleFavorite(PlacesSearchResult place) {
  ...
  Widget _buildTrendingDestination(Place destination) {
    ...
    if (destination.photos.isNotEmpty) {
      // Access TrendingDestinationsProvider methods directly from the ref.read(provider)
      final placesService = ref.read(placesServiceProvider.notifier);
      photoUrl = placesService.getPhotoUrl(destination.photos.first);
    }
  ...
  // Helper method to get a description based on price level
  String _getPriceRangeDescription(int priceLevel) {
    switch (priceLevel) {
      case 1:
        return 'Inexpensive';
      case 2:
