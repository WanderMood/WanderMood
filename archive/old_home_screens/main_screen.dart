...
import 'dart:async';
import 'package:wandermood/features/home/presentation/screens/today_screen.dart';
...
class _MainScreenState extends ConsumerState<MainScreen> with SingleTickerProviderStateMixin {
  ...
  bool _showNavigationBar = true;  // Always show navigation bar
  ...
  void initState() {
    ...
    // Process navigation extras
    if (widget.extra != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        // Show navigation bar if requested
        if (widget.extra!['showNavigationBar'] == true) {
          setState(() {
        ...
        if (widget.extra!['selectedIndex'] != null) {
          setState(() {
            _selectedIndex = widget.extra!['selectedIndex'];
          });
        }
      
        // Check if we should show saved plans tab
        if (widget.extra!['showSavedPlanSuccess'] == true) {
          setState(() {
            _selectedIndex = 1; // Index of saved plans tab
          });
  ...
  Widget build(BuildContext context) {
    ...
    // Debug info
    print('MAIN SCREEN BUILD: showNavigationBar = $_showNavigationBar');
    print('MAIN SCREEN BUILD: extra = ${widget.extra}');
    
    // FORCE NAVIGATION BAR TO SHOW ALWAYS
    _showNavigationBar = true;
    
    return locationAsync.when(
      data: (location) => _buildMainContent(location),
      loading: () => const CircularProgressIndicator(),
      error: (error, stack) => Text('Error: $error'),
    );
  }

  Widget _buildMainContent(AppLocation? location) {
  ...
  Widget _buildTopBar(
    AsyncValue<AppLocation?> locationAsync,
